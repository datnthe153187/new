...
def ps_10h15(data):
    ...
    for i in tqdm(range(5, len(data))):
        ...
        if (data.Date.iloc[i]).hour == 14 and (data.Date.iloc[i]).minute==45:
            if buyl != 0:
                am = 0
...
if __name__ == '__main__':
 #while True:
    ...
    #     mess = 'PS_10h15 enter Short, pos: -1, Enter price: '+ str(buys)[:6]
    #     mes(mess)
    #     pnl_report.to_csv('T:/alpha_live_pos/DAT/PS10h15_DAT.csv')
    # if position[-1] == 1:
    #     position_report(position)
    #     mess = 'PS_10h15 enter Long, pos: 1, Enter price: ' + str(buyl)[:6]
    #     mes(mess)
    #     pnl_report.to_csv('T:/alpha_live_pos/DAT/PS10h15_DAT.csv')
