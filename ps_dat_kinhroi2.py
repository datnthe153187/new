...
def kinhroi(data):
    ...
    for i in tqdm(range(len(signal))):
        ...
        if buy != 0 and (signal.Date.iloc[i]).hour == 14 and (signal.Date.iloc[i]).minute==30:
            ...
            hi = 0
        if hi == 0 and buy1 != 0:
            if ((signal.Date.iloc[i]).hour == 9 and (signal.Date.iloc[i]).minute==00 and signal.Open.iloc[i]-buy1 < -7) or (signal.Date.iloc[i]).hour == 10 and (signal.Date.iloc[i]).minute==00:
                pos = position[-1]
                position.append(0)
...
if __name__ == '__main__':
    ...
    if position[-1] == 1:
        mess = 'profit_short(live):' + str(profit[-1])[:5]
        mess = mess + ', DAT_PS_long: pos: 40, Enter price: ' + str(buy1)[:6]
        mes(mess)
        posi(info)
