...
if __name__ == '__main__':
    while True:
        if datetime.datetime.now().hour == 10 and datetime.datetime.now().minute == 15 and datetime.datetime.now().second == 5:
            ...
            if position[-1] == -1:
                mess = 'DAT_PS_short, pos: -1, enter price short: '+ str(buy)[:6]
                mes(mess)
            if position[-1] == 0:
                mess = 'DAT_PS_short, pos: 0, hom nay khong co lenh short'
                mes(mess)
            time.sleep(60)
        if datetime.datetime.now().hour == 16 and datetime.datetime.now().minute == 55 and datetime.datetime.now().second == 50:
            data = test_live(15)
            pnl, profit, position, short, buy, buy1, pos, pnl_report = kinhroi(data)
            if position[-1] == 1:
                mess = 'DAT_PS_long: pos: 1, enter price long: ' + str(buy1)[:6]
                mess = mess + '. profit_short:' + str(profit[-1])[:5]
                mes(mess)
            time.sleep(60)
        if datetime.datetime.now().hour == 9 and datetime.datetime.now().minute == 12 and datetime.datetime.now().second == 5:
            data = test_live(15)
            pnl, profit, position, short, buy, buy1, pos, pnl_report = kinhroi(data)
            if pos != position[-1]:
