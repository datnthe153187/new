...
def kinhroi(data):
    ...
    return pnl, profit, position, short, buy, buy1, pos
...
if __name__ == '__main__':
 #while True:
    ...
    if position[-1] == -1:
        mess = 'DAT_PS_short(live), pos: -40, Enter price: '+ str(buy)[:6]
        mes(mess)
        posi(info)
        position_report(position)
    if position[-1] == 0:
        mess = 'DAT_PS_short(live), pos: 0, hom nay khong co lenh short'
        mes(mess)
        position_report(position)
    time.sleep(60)
