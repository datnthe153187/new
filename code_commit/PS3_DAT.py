...
if __name__ == '__main__':
 #while True:
    ...
    if short.pos.iloc[-1] == -1:
        if short.pos.iloc[-1] != short.pos.iloc[-2]:
            mess = 'PS3_DAT, pos: -1, enter price: ' + str(buys)[:5] 
            mes(mess)
        else:
            mess = 'PS3_DAT, pos: -1'
            mes(mess)
    if short.pos.iloc[-1] == 1:
        if short.pos.iloc[-1] != short.pos.iloc[-2]:
            mess = 'PS3_DAT, pos: 1, enter price: ' + str(buyl)[:5] 
            mes(mess)
        else:
            mess = 'PS3_DAT, pos: 1'
            mes(mess)
    pnl_report.to_csv('T:/alpha_live_pos/DAT/PS3_DAT.csv')
